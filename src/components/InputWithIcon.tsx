...
export type ComponentPropType = {
    settings: {
        ...
        icon: IconType,
        autofocus?: boolean,
        onInputFn: ((e:SyntheticEvent<HTMLInputElement>) => void) | (() => void)
    }, rightButton?: {
        show: boolean,
        icon: IconType,
        onclickFn: () => void
            ...
            <ul>
                {
                    errors?.map((error, index) => {
                        return (
                            <li className="text-red-700 text-sm" key={index}>{error}</li>
                        )
                    })
                }
            </ul>
