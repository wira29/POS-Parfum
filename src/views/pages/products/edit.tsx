      ...
      try {
        ...
        if (data.category) {
          if (typeof data.category === 'object' && data.category !== null) {
            ...
            const cat = categories.find(c => String(c.value) === String(data.category));
            if (cat) setSelectedCategoryName(cat.label);
          }
        } else {
          setCategory("");
        ...
        // Set default values if single variant
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hasVariant) {
      ...
      variantMatrix.forEach((variant, i) => {
        ...
        const options = variant.volumes && variant.volumes.filter(Boolean).length > 0 
        ...
        options.forEach((option, j) => {
          ...
          formData.append(`product_details[${detailIdx}][unit_id]`, variantUnits[i]?.[j] || "");
