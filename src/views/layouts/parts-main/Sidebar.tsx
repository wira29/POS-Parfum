                            ...
                            {(group.children || [group]).map((item, idx) => {
                                ...
                                return (
                                    <li key={idx}>
                                        <Link
                                            to={item.path}
                                            className={`flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-blue-100 text-sm font-medium transition-all duration-300 ${isActive ? 'bg-blue-600 text-white' : 'text-gray-700'} ${isCollapsed ? 'justify-center' : ''}`}
                                        >
